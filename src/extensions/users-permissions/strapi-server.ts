...
const verifyRefreshToken = (token) => {
  ...
  console.log("verificar token ", process.env.REFRESH_SECRET);
...
export default (plugin) => {
  plugin.controllers.auth.callback = async (ctx) => {
    ...
    if (provider === "local") {
      ...
      if (!validPassword) {
        ...
        user.refreshToken = issueRefreshToken({ id: user.id });
  ...
  plugin.controllers.auth["refreshToken"] = async (ctx) => {
    ...
    try {
      const obj: any = await verifyRefreshToken(refreshCookie);
      console.log({ obj });
      const user = await strapi
        .query("plugin::users-permissions.user")
